...
router.post("/restaurant_submit", upload.any(), function (req, res, next) {
  pool.query(
    "insert into restaurants(restaurantname, ownername, phonenumber, emailid, mobileno, url, fssai, gstno, gsttype, filefssai, fileshopact, filelogo, address, stateid, cityid, createdat, updatedat,password)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    [
...
router.post("/restaurant_edit_data", upload.any(), function (req, res, next) {
  pool.query(
    "update  restaurants set restaurantname=?, ownername=?, phonenumber=?, emailid=?, mobileno=?, url=?, fssai=?, gstno=?, gsttype=?,  address=?, stateid=?, cityid=?,  updatedat=? where restaurantid=?",
    [
      req.body.restaurantname,
      req.body.ownername,
      req.body.phonenumber,
    ...
    function (error, result) {
      if (error) {
        ...
        res
          .status(200)
          .json({ status: true, message: "Restaurant Updated Successfully" });
      }
    }
  );
});
    ...
    function (error, result) {
      if (error) {
        ...
        res
          ...
          .json({
            status: true,
            message: "Fssai Certificate Updated Successfully",
          });
      }
    }
  );
      ...
      function (error, result) {
        if (error) {
          ...
          res
            ...
            .json({
              status: true,
              message: "ShopAct Certificate Updated Successfully",
            });
        }
      }
    );
  ...
  pool.query(
    ...
    function (error, result) {
      if (error) {
        ...
        res
          .status(200)
          .json({ status: true, message: "Logo Updated Successfully" });
      }
    }
  );
});
